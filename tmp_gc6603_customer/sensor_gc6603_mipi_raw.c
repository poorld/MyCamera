...
#define LOG_TAG "gc6603_sharkl2"


#include "sensor_gc6603_mipi_raw.h"


#define Android13_Version  1   //android13Èú¢„Ë¶ÅÁâàÊú¨ÊâìÂº¢„Ê≠§ÂÆèÊé??
#define REG_0 0x0914
#define REG_1 0x0915
...
static void gc6603_drv_write_reg2sensor(cmr_handle handle, struct sensor_i2c_reg_tab *reg_info) 
{
 ...
 * please modify this function acording your spec
 *============================================================================*/
cmr_u8 regValTable[29][6] = {   
//0914, 0915, 0225, 0e67, 0e68, 0242    |  CG |  µº ±∂ ˝|Again dB|
...
static 	int total = sizeof(gainLevelTable) / sizeof(cmr_u32);
							  
static void gc6603_drv_write_gain(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 gain) 
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	int i;
	cmr_u32 tol_dig_gain = 0;
	...
	for(i = 0; i < total; i++)
	{
	   if((gainLevelTable[i] <= gain)&&(gain < gainLevelTable[i+1]))
		...
		tol_dig_gain = gain*1024/gainLevelTable[i];	
		aec_info->again->settings[0].reg_value=regValTable[i][0];
		aec_info->again->settings[1].reg_value=regValTable[i][1];
		aec_info->again->settings[2].reg_value=regValTable[i][2];
		aec_info->again->settings[3].reg_value=regValTable[i][3];
		aec_info->again->settings[4].reg_value=regValTable[i][4];
		aec_info->again->settings[5].reg_value=regValTable[i][5];


	  aec_info->again->settings[6].reg_value = (tol_dig_gain >> 8) & 0xff;
	  aec_info->again->settings[7].reg_value = (tol_dig_gain& 0xff);
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc6603_drv_write_frame_length(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info, cmr_u32 frame_len)
{
    SENSOR_IC_CHECK_PTR_VOID(aec_info);
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static void gc6603_drv_write_shutter(cmr_handle handle, struct sensor_aec_i2c_tag *aec_info , cmr_u32 shutter)
{
	SENSOR_IC_CHECK_PTR_VOID(aec_info);
    SENSOR_IC_CHECK_HANDLE_VOID(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	if(shutter > 16367) shutter = 16367;
 ...
 *============================================================================*/
static void gc6603_drv_calc_exposure(cmr_handle handle, cmr_u32 shutter,cmr_u32 dummy_line, 
                                                  cmr_u16 mode, struct sensor_aec_i2c_tag *aec_info)
{
    ...
    dest_fr_len = (dest_fr_len > 16383) ? 16383 : dest_fr_len;
	...
	reg_fr_len = (frame_len_h << 8) | frame_len_l;																 

	if (dest_fr_len != reg_fr_len){
		sns_drv_cxt->sensor_ev_info.preview_framelength = dest_fr_len;
		gc6603_drv_write_frame_length(handle, aec_info, dest_fr_len);
	}
    sns_drv_cxt->sensor_ev_info.preview_shutter = shutter;
	gc6603_drv_write_shutter(handle, aec_info, shutter);
...
static void gc6603_drv_calc_gain(cmr_handle handle,cmr_uint isp_gain, struct sensor_aec_i2c_tag *aec_info) 
{
	SENSOR_IC_CHECK_HANDLE_VOID(handle);
	struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	gc6603_drv_write_gain(handle, aec_info,sensor_gain);
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_power_on(cmr_handle handle, cmr_uint power_on) 
{
    ...
    BOOLEAN power_down = g_gc6603_mipi_raw_info.power_down_level;
    BOOLEAN reset_level = g_gc6603_mipi_raw_info.reset_pulse_level;	
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_init_fps_info(cmr_handle handle) 
{
...
static cmr_int gc6603_drv_get_static_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
    ...
    if (!fps_info->is_init) {
        gc6603_drv_init_fps_info(handle);
    }
    ...
    ex_info->name = (cmr_s8 *)g_gc6603_mipi_raw_info.name;
    ex_info->sensor_version_info = (cmr_s8 *)g_gc6603_mipi_raw_info.sensor_version_info;
...
static cmr_int gc6603_drv_get_fps_info(cmr_handle handle, cmr_u32 *param) 
{
    cmr_int rtn = SENSOR_SUCCESS;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_access_val(cmr_handle handle, cmr_uint param)
{
	...
	SENSOR_LOGI("sensor gc6603: param_ptr->type=%x", param_ptr->type);
	
	switch(param_ptr->type)
	{
		case SENSOR_VAL_TYPE_GET_STATIC_INFO:
			ret = gc6603_drv_get_static_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_GET_FPS_INFO:
			ret = gc6603_drv_get_fps_info(handle, param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_SET_SENSOR_CLOSE_FLAG:
			ret = sns_drv_cxt->is_sensor_close = 1;
			break;
		case SENSOR_VAL_TYPE_GET_PDAF_INFO:
			//ret = gc6603_drv_get_pdaf_info(handle, param_ptr->pval);
			break;
		default:
			break;
    }
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_identify(cmr_handle handle, cmr_uint param)
{
	...
	pid_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC6603_PID_ADDR);

	if (GC6603_PID_VALUE == pid_value) {
		ver_value = hw_sensor_read_reg(sns_drv_cxt->hw_handle, GC6603_VER_ADDR);
		SENSOR_LOGI("Identify: pid_value = %x, ver_value = %x", pid_value, ver_value);
		if (GC6603_VER_VALUE == ver_value) {
			SENSOR_LOGI("this is gc6603(YH&YJ&YK&YC) sensor");
 ...
 *============================================================================*/
static cmr_int gc6603_drv_before_snapshot(cmr_handle handle, cmr_uint param)
{
...
*/
    ...
    gc6603_drv_calc_exposure(handle,cap_shutter, 0 , capture_mode,&gc6603_aec_info);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.frame_length);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.shutter);

	sns_drv_cxt->sensor_ev_info.preview_gain=cap_gain;
	gc6603_drv_write_gain(handle, &gc6603_aec_info, cap_gain);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.again);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.dgain);
 ...
 *============================================================================*/
static cmr_int gc6603_drv_write_exposure(cmr_handle handle, cmr_uint param)
{
	...
	gc6603_drv_calc_exposure(handle,exposure_line, dummy_line, size_index, &gc6603_aec_info);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.frame_length);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.shutter);
 ...
 * you can change this function if it's necessary
 *============================================================================*/
static cmr_int gc6603_drv_write_gain_value(cmr_handle handle, cmr_uint param)
{
	cmr_int ret_value = SENSOR_SUCCESS;
	...
	gc6603_drv_calc_gain(handle,param, &gc6603_aec_info);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.again);
	gc6603_drv_write_reg2sensor(handle, gc6603_aec_info.dgain);
 ...
 * please don't change this function unless it's necessary
 *============================================================================*/
static cmr_int gc6603_drv_read_aec_info(cmr_handle handle, cmr_uint param) 
{
    cmr_int ret_value = SENSOR_SUCCESS;
    ...
    info->aec_i2c_info_out = &gc6603_aec_info;
    exposure_line = info->exp.exposure;
    dummy_line = info->exp.dummy;
    mode = info->exp.size_index;

    gc6603_drv_calc_exposure(handle, exposure_line, dummy_line, mode, &gc6603_aec_info);
    gc6603_drv_calc_gain(handle,info->gain, &gc6603_aec_info);

    return ret_value;
}

static cmr_int gc6603_drv_set_master_FrameSync(cmr_handle handle, cmr_uint param) 
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_stream_on(cmr_handle handle, cmr_uint param)
{
    SENSOR_IC_CHECK_HANDLE(handle);
    struct sensor_ic_drv_cxt * sns_drv_cxt = (struct sensor_ic_drv_cxt *)handle;
	...
	//gc6603_drv_set_master_FrameSync(handle, param);
	//gc6603_drv_set_slave_FrameSync(handle, param);
#endif

	usleep(100 * 1000);
	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0100, 0x09);//on
 ...
 * please modify this function acording your spec
 *============================================================================*/
static cmr_int gc6603_drv_stream_off(cmr_handle handle, cmr_uint param)
{
	SENSOR_LOGI("E");
    ...
    if (!sns_drv_cxt->is_sensor_close) {
  	...
  	hw_sensor_write_reg(sns_drv_cxt->hw_handle, 0x0100, 0x08);//off 
...
static cmr_int gc6603_drv_handle_create(struct sensor_ic_drv_init_para *init_param, cmr_handle* sns_ic_drv_handle) 
{
	...
	//gc6603_drv_write_frame_length(sns_drv_cxt, &gc6603_aec_info, sns_drv_cxt->sensor_ev_info.preview_framelength);
	gc6603_drv_write_gain(sns_drv_cxt, &gc6603_aec_info, sns_drv_cxt->sensor_ev_info.preview_gain);
	gc6603_drv_write_shutter(sns_drv_cxt, &gc6603_aec_info, sns_drv_cxt->sensor_ev_info.preview_shutter);

    sensor_ic_set_match_module_info(sns_drv_cxt, ARRAY_SIZE(s_gc6603_module_info_tab), s_gc6603_module_info_tab);
    sensor_ic_set_match_resolution_info(sns_drv_cxt, ARRAY_SIZE(s_gc6603_resolution_tab_raw), s_gc6603_resolution_tab_raw);
    sensor_ic_set_match_trim_info(sns_drv_cxt, ARRAY_SIZE(s_gc6603_resolution_trim_tab), s_gc6603_resolution_trim_tab);
    sensor_ic_set_match_static_info(sns_drv_cxt, ARRAY_SIZE(s_gc6603_static_info), s_gc6603_static_info);
    sensor_ic_set_match_fps_info(sns_drv_cxt, ARRAY_SIZE(s_gc6603_mode_fps_info), s_gc6603_mode_fps_info);

    /*init exif info,this will be deleted in the future*/
    gc6603_drv_init_fps_info(sns_drv_cxt);

    /*add private here*/
    return ret;
}

static cmr_int gc6603_drv_handle_delete(cmr_handle handle, void *param) 
{
    cmr_int ret = SENSOR_SUCCESS;
...
static cmr_int gc6603_drv_get_private_data(cmr_handle handle, cmr_uint cmd, void**param)
{
    cmr_int ret = SENSOR_SUCCESS;
    SENSOR_IC_CHECK_HANDLE(handle);
    SENSOR_IC_CHECK_PTR(param);
 ...
 * .power = gc6603_power_on,
 *============================================================================*/
static struct sensor_ic_ops s_gc6603_ops_tab = {
    .create_handle = gc6603_drv_handle_create,
    .delete_handle = gc6603_drv_handle_delete,
    .get_data = gc6603_drv_get_private_data,
/*---------------------------------------*/
    .power = gc6603_drv_power_on,
    .identify = gc6603_drv_identify,
    .ex_write_exp = gc6603_drv_write_exposure,
    .write_gain_value = gc6603_drv_write_gain_value,

#if Android13_Version

#endif
	.read_aec_info = gc6603_drv_read_aec_info,
    .ext_ops = {
            [SENSOR_IOCTL_BEFORE_SNAPSHOT].ops = gc6603_drv_before_snapshot,
            [SENSOR_IOCTL_STREAM_ON].ops = gc6603_drv_stream_on,
            [SENSOR_IOCTL_STREAM_OFF].ops = gc6603_drv_stream_off,
            /* expand interface,if you want to add your sub cmd ,
             *  you can add it in enum {@SENSOR_IOCTL_VAL_TYPE}
             */
            [SENSOR_IOCTL_ACCESS_VAL].ops = gc6603_drv_access_val,
    }
};
